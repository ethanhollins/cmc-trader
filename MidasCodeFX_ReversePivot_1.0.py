...
def onTrigger(shift):
	...
	if isNumStrands(1):
		...
		long_triggers = [trigger for trigger in current_triggers if trigger.direction == Direction.LONG and trigger.is_regular]
		short_triggers = [trigger for trigger in current_triggers if trigger.direction == Direction.SHORT and trigger.is_regular]
		...
		if pos_direction == None:
			if isNumValidStrands(2, Direction.LONG):
				if len(long_triggers) <= 0:
					t = Trigger(Direction.LONG, tradable = True, is_regular = True)
			...
			if isNumValidStrands(2, Direction.SHORT):
				if len(short_triggers) <= 0:
					t = Trigger(Direction.SHORT, tradable = True, is_regular = True)
					current_triggers.append(t)
