...
def runSequence(shift):
	...
	for st in sub_triggers:
		...
		if st.direction == Direction.LONG:
			if st.start < st.prev_trigger.getFibPercent(73):
				print("delete 73")
	...
	if not trend.trend_state == TrendState.COMPLETE:
		...
		if trend.trend_state == TrendState.ONE or trend.trend_state == TrendState.TWO:
			...
			if (not trend.trigger.is_sub 
				and ( (not 73 in trend.trigger.fib_points and not 62 in trend.trigger.fib_points)
					or 73 in trend.trigger.fib_active or 62 in trend.trigger.fib_active )):
...
def deleteAllOtherTriggers(trigger):
	for i in range(len(current_triggers) - 1, -1, -1):
		...
		if (t == trigger.direction and t != trigger 
			and (not 73 in t.fib_points 
				or (current_triggers.index(current_triggers[i]) < current_triggers.index(trigger) 
...
def report():
	...
	if entry_trigger_l:
		print("entry_trigger_l", "("+str(entry_trigger_l.count)+"):", str(entry_trigger_l.fib_points))
		print(entry_trigger_l.fib_active)
		print(str(entry_trigger_l.fibonacci_r_state)+", "+str(entry_trigger_l.fibonacci_m_state))
		print("\n")

	if entry_trigger_s:
		print("entry_trigger_s", "("+str(entry_trigger_l.count)+"):", str(entry_trigger_s.fib_points))
